...
class YoloOnnxProvider:
    """CPU-first YOLO ONNX reference plugin with no control-side effects."""
    ...
    def __init__(
        ...
        *,
        ...
        if device.lower() != "cpu":
            raise ValueError("CV V1 only accepts device=cpu")
    ...
    def analyze(
        ...
        for row in output[0]:
            x1, y1, x2, y2, confidence, raw_class = row
        ...
        return CVObservation(
            ...
            values={
                ...
                "frame_age_ms": round(max(0.0, time.time() * 1000.0 - timestamp_ms), 1),
    ...
    def _load(self) -> None:
        ...
        if model_input.shape[-2:] != [self.image_size, self.image_size]:
            raise ValueError(
                f"YOLO input is {model_input.shape[-2:]}, expected "
